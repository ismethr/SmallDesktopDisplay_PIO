...
需要了解连接情况时，在电脑浏览器打开 [本机状态页面](http://127.0.0.1:8766/)，即可查看 USB 连接、系统指标、Codex 数据更新时间和出口位置。页面只读、每秒自动刷新，不影响小屏工作，也不需要让 App 常驻 Dock。
...
- 自动或手动城市代码，显示定时获取的天气、室外温度、室外湿度、风向风力、当日高低温和天气图标。
...
4. 联网后依次同步 NTP 与天气，然后关闭 Wi-Fi 射频；此后每 30 分钟自动重复，也可通过串口发送 `0x08` 立即刷新。
...
本地构建产物与 GitHub Actions 的 `SmallDesktopDisplayBridge-macos-x86_64`、`SmallDesktopDisplayBridge-macos-arm64` 均包含可直接运行的 `MiniDisplay Bridge.app`。日志、登录启动与首次打开方式见 [`tools/desktop_display_bridge/README.md`](tools/desktop_display_bridge/README.md)。
...
| `WEATHER_REFRESH_INTERVAL_MINUTES` | `30` | 自动联网刷新 NTP 与天气的间隔分钟数，可设为 `1-1440` |
