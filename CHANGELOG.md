...
- MiniDisplay Bridge 1.12.0 新增 Windows 安装包、托盘菜单、可选登录自启、正常退出及重复启动打开面板；修复 64 位句柄关闭、日志目录失败无提示和启动参数错误被静默吞掉的问题。
- Windows 温度通过 LibreHardwareMonitor 0.9.6 库的独立只读采集程序接入，接口限定本机、随 Bridge 退出；兼容旧版 WMI 与 NVIDIA nvidia-smi。安装包附带官方组件、第三方许可及对应源码。CPU 传感器需安装官方 PawnIO，不使用 ACPI 区域温度冒充 CPU 温度。
- USB 状态屏连续 4 秒断联自动切到时钟，重连恢复状态界面；独立 CRC 校验的 MSC1 本地时间同步包兼容旧固件。新增午夜、millis 回绕、重连整屏恢复和 Windows 生命周期测试。
- 断联时钟直接复用天气时钟的 LineAtom 字库、白色小时、黄色分钟和右侧小秒数；两个固件共享同一个绘制实现。Windows 温度排除 Intel Distance to TjMax 差值传感器及 GPU 显存温度，避免把它们误报为核心温度。
- 断联页保留完整天气图标、城市/AQI、温湿度、中文日期和初音动画。电脑定期同步并缓存天气，小屏无需 Wi-Fi；新增 MSC2 日期同步与 MSW1 天气包，恢复连接后自动切回系统状态。

- MiniDisplay Bridge 1.11.0 新增原生 macOS 菜单栏图标：左键打开内置状态面板，右键直达设置或退出；关闭窗口保持后台运行，退出时一并结束工作进程。
- 新增可持久化的日间/夜间/断线亮度、夜间时段与 USB 端口设置，支持在线应用和手动重连，无需修改状态屏固件。夜间和断线亮度不再反向调亮屏幕。
- 设置接口仅允许本机来源，增加随机会话校验、请求体限制、严格字段校验、并发修订号和原子保存；新增持久化恢复、保存失败回滚、非法请求、端口切换及实时亮度测试。
- USB 状态屏重新分配 240 × 240 布局：CPU/内存并排大字、温度独立成行、Codex 明确区分等待/剩余/缓存，网络保留国旗与上下行三栏；仅重绘变化区域，保留已经实机确认的国旗色序。
- MiniDisplay Bridge 1.10.0 新增本机只读状态页面，集中展示 USB、系统指标、Codex 更新时间与出口位置，支持窄窗口；成功轮询不再写入访问日志。
- 加强串口初始化失败清理、完整写入检查和自动重连，避免同步排空串口阻塞；系统采集异常自动恢复，过期快照停止发送，有效快照在慢采样时维持心跳。
- 新增使用固件绘图代码与真实字库的布局回归检查，以及采样恢复、串口短写/初始化失败/退出、数据过期和本机页面安全边界测试。
- macOS 后台程序正式命名为 `MiniDisplay Bridge（迷你屏桥接）`，新增原创的小屏监控与 USB-C 桥接图标；图标覆盖 16–1024 像素尺寸，并继续保留原发布 ZIP 文件名以兼容现有下载链接和自动发布流程。
- 修复 USB 状态屏系统卡片中 32 像素高数值与 8 像素标签互相覆盖的问题；改用严格分层的 16 像素数值行，并将网络位置改为小国旗加地区代码，极端网速也会自动使用 `G/s` 避免跨栏。国旗颜色按实机观察单独校正红蓝通道，不改变其余主题色。
...
- USB 状态屏升级为紧凑双行系统布局，新增 CPU 最热点、GPU 温度和公网出口国家/地区短标签；macOS App 内置无需管理员权限的只读 AppleSMC 读取器，并参考 Here for macOS 每 30 秒更新出口位置、失败时灰显上次成功值。`MSD4` 为缺失数据保留明确占位，固件继续兼容 `MSD3`。
