...
- 新增第二块 ESP8266 的 USB Mac 状态屏固件，显示 CPU、内存、CPU 温度和默认网卡实时速度，不保存 Wi-Fi 配置。
- 将原 Codex 用量桥接扩展为统一后台，在保持天气时钟 `8766` 接口兼容的同时，每秒通过带 CRC16 的有界串口协议驱动状态屏。
- 新增 USB 自动重连、断线提示、默认路由网卡选择、持续 macmon 温度读取、诊断接口及 Python/C++ 自动测试。
- 状态屏背景改为中性黑色，并新增默认 00:00–07:00 夜间调暗、数据断开调暗和恢复连接自动恢复亮度；三个亮度值及夜间时段均可由统一后台配置。
